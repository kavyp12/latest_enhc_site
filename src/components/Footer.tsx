...
export function Footer() {
  ...
  useEffect(() => {
    const listItems = listItemsRef.current; // Store ref.current
    const spanItems = spanItemsRef.current; // Store ref.current
    ...
    listItems.forEach(addEventListeners);
    spanItems.forEach(addEventListeners);

    return () => {
      listItems.forEach(removeEventListeners);
      spanItems.forEach(removeEventListeners);
    };
  }, []);
  ...
  return (
    <footer className="flex relative flex-col container py-12 h-screen justify-evenly">
      <div className="flex flex-col">
        <ul className="flex flex-col gap-5 uppercase w-24">
          {[
            ...
            >
              <span className="block initial absolute top-0 left-0 w-full h-full">
                {item.text}
              </span>
              <span className="block hover absolute top-0 left-0 w-full h-full">
                {item.text}
              </span>
            </li>
          ))}
      ...
      >
        <h1 className="w-full text-[12vw] uppercase leading-none">
          Let&apos;s Talk
        </h1>
      ...
      <div className="w-full flex flex-col md:flex-row gap-10 justify-between">
        <div className="flex gap-10 uppercase">
          <div className="relative overflow-hidden group/line cursor-pointer">
            <a
              href="mailto:infoenhc@gmail.com"
              className="leading-none pb-2"
            >
              mail
            </a>
